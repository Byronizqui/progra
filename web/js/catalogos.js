...
function dibujarFilaCatalogos(rowData) {
    ...
    row.append($('<td><button type="button" class="btn btn-default btn-xs" aria-label="Left Align" onclick="editarCatalogos('+rowData.CId+');">'+
                    ...
                    '</button>'+
                    '<button type="button" class="btn btn-default btn-xs" aria-label="Left Align" onclick="eliminarCatalogo('+rowData.CId+');">'+
    ...
    $.ajax({
        ...
        success: function (data) { //si todo esta correcto en la respuesta del ajax, la respuesta queda en el data
           ...
           // ocultarModal("myModal");
            ...
            alert("El catálogo está listo para ser editado");
        ...
        $.ajax({
            ...
            success: function (data) { //si todo esta correcto en la respuesta del ajax, la respuesta queda en el data
                ...
                if (tipoRespuesta === "C~") {
                    ...
                    consultarCatalogos();
                    setTimeout(consultarCatalogos, 3000);
                } else {
                    if (tipoRespuesta === "E~") {
                        $("#myModalFormulario2").modal("hide");
