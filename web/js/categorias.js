...
function dibujarFilaCategorias(rowData) {
    ...
    row.append($('<td><button type="button" class="btn btn-default btn-xs" aria-label="Left Align" onclick="editarCategoria('+rowData.CId+');">'+
                    ...
                    '</button>'+
                    '<button type="button" class="btn btn-default btn-xs" aria-label="Left Align" onclick="eliminarCategoria('+rowData.CId+');">'+
    ...
    $.ajax({
        ...
        success: function (data) { //si todo esta correcto en la respuesta del ajax, la respuesta queda en el data
           ...
           // ocultarModal("myModal");
            ...
            alert("La categoría está lista para ser editada");
...
function eliminarCategoria(ide){
    $.ajax({
        ...
        success: function (data) { //si todo esta correcto en la respuesta del ajax, la respuesta queda en el data
            ...
            consultarCategorias();
            setTimeout(consultarCategorias, 3000);// hace una pausa y consulta la información de la base de datos
